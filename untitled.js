...
d3.json("data.json", function(err, graph) {
	...
	var display = function() {
		...
		displayGraph(filteredNodes, filteredLinks);
	}
	...
	function displayGraph(n, l) {
		var link = svg.append("g").attr("class", "links").selectAll("line").data(l).enter().append("line");
		var node = svg.append("g").attr("class", "nodes").selectAll("g").data(n).enter().append("g");
		var circles = node.append("circle").attr("r", 5).attr("fill", function(d) { 
	  	...
	  	simulation.nodes(n).on("tick", ticked);
